...
const ExcellencyEditor = (props: Props) => {
  const { character, onChange, onChangeMulti } = props
  const { excellency, excellency_stunt } = character
  const [isOpen, setOpen, setClosed] = useDialogLogic()

  const handleExcellencyChange = (e: ChangeEvent<HTMLInputElement>) => {
    const name = e.target.name as 'excellency' | 'excellency_stunt'
    const value = e.target.value as unknown as Character[typeof name][]
  ...
  return (
    <>
      <Button onClick={setOpen}>Excellencies</Button>
