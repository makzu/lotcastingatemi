...
export default function HealthLevelBoxes(props) {
  ...
  for (var i = 0; i < totalHealthLevels; i++) {
    ...
    if (aggDamage > 0) {
      box = <div key={ i } className="damageBoxWrapper">
        <AggravatedDamageBox level={ level } />
        <span className="damageBoxLevel">{ level }</span>
      </div>
      aggDamage--
    } else if (lthDamage > 0) {
      box = <div key={ i } className="damageBoxWrapper">
        <LethalDamageBox level={ level } />
        <span className="damageBoxLevel">{ level }</span>
      </div>
      lthDamage--
    } else if (bshDamage > 0) {
      box = <div key={ i } className="damageBoxWrapper">
        <BashingDamageBox level={ level } />
        <span className="damageBoxLevel">{ level }</span>
      </div>
      bshDamage--
    } else {
      box = <div key={ i } className="damageBoxWrapper">
        <EmptyDamageBox level={ level } />
        <span className="damageBoxLevel">{ level }</span>
      </div>
    }
