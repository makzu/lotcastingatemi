...
const ShapeNecromancyWidget = ({ character, children, classes }: Props) => {
  ...
  const [total, setTotal] = useState(character.sorcerous_motes)
  ...
  const updateRoll = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTotal(parseInt(e.target.value, 10) + character.sorcerous_motes)
  }

  const updateTotal = (e: React.ChangeEvent<HTMLInputElement>) => {
    setMotes(parseInt(e.target.value, 10) - character.sorcerous_motes)
    setTotal(parseInt(e.target.value, 10))
  }

  const submit = () => {
    close()
    dispatch(updateAction(character.id, { sorcerous_motes: total }))
  }
