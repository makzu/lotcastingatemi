...
YAML.safe_load_file('db/seeds/characters.yaml').each do |c|
...
YAML.safe_load_file('db/seeds/merits.yaml').each do |m|
  merit = Merit.find_or_initialize_by(id: m['id'])
  merit.update(m)
  puts "Updated merit #{merit.label.presence || merit.merit_name} for #{merit.character.name}"
end

puts 'Reloading weapons'
YAML.safe_load_file('db/seeds/weapons.yaml').each do |w|
...
YAML.safe_load_file('db/seeds/charms.yaml').each do |c|
...
puts 'Reloading QCs'
YAML.safe_load_file('db/seeds/qcs.yaml').each do |q|
...
YAML.safe_load_file('db/seeds/qc_merits.yaml').each do |m|
...
YAML.safe_load_file('db/seeds/qc_charms.yaml').each do |c|
...
YAML.safe_load_file('db/seeds/battlegroups.yaml').each do |a|
...
YAML.safe_load_file('db/seeds/qc_attacks.yaml').each do |a|
