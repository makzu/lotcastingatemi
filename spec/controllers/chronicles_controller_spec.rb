	...
	describe "POST #create" do
	    context "With valid attributes" do
	      it "Increases Chronicle count by 1" do
	        ...
	        @chronicle_params = FactoryGirl.attributes_for(:chronicle, st_id: @player.id)
  	...
  	describe "PATCH #update" do
	    context "With valid attributes" do
	      it "Updates character attributes" do
	        @player = FactoryGirl.create(:player)
	        @chronicle = FactoryGirl.create(:chronicle)
	        @updated_chronicle_params = FactoryGirl.attributes_for(:chronicle, st_id: @player.id)

	        expect(@chronicle.st_id).not_to eq(@player.id)

	        patch :update, params: { id: @chronicle.id, chronicle: @updated_chronicle_params, format: :json}
	        @chronicle.reload

	        expect(@chronicle.st_id).to eq(@player.id)
	      end
	    end
	    ...
	    context "With invalid attributes" do
	      it "Updates character attributes" do
	        @player = FactoryGirl.create(:player)
	        @chronicle = FactoryGirl.create(:chronicle)
	        @invalid_updated_chronicle_params = FactoryGirl.attributes_for(:chronicle, st_id: "Invalid Attribute")
